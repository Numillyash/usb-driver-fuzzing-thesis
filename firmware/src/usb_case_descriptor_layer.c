...
static bool usb_case_is_composite_cdc_msc_persona(void)
{
...
usb_case_descriptor_profile_t usb_case_get_descriptor_profile(void)
{
    ...
    } else if (usb_case_is_composite_cdc_msc_persona()) {
...
#if defined(CFG_TUD_CDC) && (CFG_TUD_CDC == 1) && defined(CFG_TUD_MSC) && (CFG_TUD_MSC == 1)
static const uint8_t usb_case_cfg_desc_composite_cdc_msc[] = {
...
static const uint8_t usb_case_cfg_desc_composite_cdc_msc_dup_itf_num[] = {
    TUD_CONFIG_DESCRIPTOR(1, 3, 0, USB_CASE_COMPOSITE_CDC_MSC_TOTAL_LEN, 0x80, 100),
    TUD_CDC_DESCRIPTOR(
        USB_CASE_ITF_NUM_CDC_COMM,
        USB_CASE_STRIDX_CDC,
        ...
        64),
    /*
     * Case 062: intentionally reuse an existing CDC interface number for MSC
     * to model duplicate bInterfaceNumber inside one configuration descriptor.
     */
    TUD_MSC_DESCRIPTOR(
        USB_CASE_ITF_NUM_CDC_DATA,
        USB_CASE_STRIDX_MSC_COMPOSITE,
        USB_CASE_ENDPOINT_MSC_COMPOSITE_OUT,
        USB_CASE_ENDPOINT_MSC_COMPOSITE_IN,
        CFG_TUD_MSC_EP_BUFSIZE),
...
static uint8_t usb_case_cfg_desc_composite_cdc_msc_bnuminterfaces_too_many[] = {
    TUD_CONFIG_DESCRIPTOR(1, 3, 0, USB_CASE_COMPOSITE_CDC_MSC_TOTAL_LEN, 0x80, 100),
    TUD_CDC_DESCRIPTOR(
        USB_CASE_ITF_NUM_CDC_COMM,
        USB_CASE_STRIDX_CDC,
...
static uint8_t const *usb_case_cfg_desc_composite_cdc_msc_bnuminterfaces_too_many_get(void)
{
    /*
     * Case 065: keep CDC+MSC baseline behavior, but advertise bNumInterfaces
     * larger than the actual interface descriptor count in this configuration.
     * Descriptor layout: [config:9 bytes], bNumInterfaces is byte 4.
     */
    enum {
        USB_CASE_CFG_BNUMINTERFACES_OFFSET = 4,
        USB_CASE_CFG_BNUMINTERFACES_TOO_MANY = 4,
    };
    static bool patched = false;

    if (!patched) {
        usb_case_cfg_desc_composite_cdc_msc_bnuminterfaces_too_many[USB_CASE_CFG_BNUMINTERFACES_OFFSET] =
            USB_CASE_CFG_BNUMINTERFACES_TOO_MANY;
        patched = true;
    }
...
#if defined(CFG_TUD_CDC) && (CFG_TUD_CDC == 1) && defined(CFG_TUD_MSC) && (CFG_TUD_MSC == 1)
static const char *const usb_case_composite_cdc_msc_strings[] = {
    "",
    "RP2040 USB Research",
    "usb_case_composite_msc_demo CDC+MSC baseline",
    "",
    "USB Case CDC",
    "",
...
int32_t tud_msc_scsi_cb(uint8_t lun, uint8_t const scsi_cmd[16], void *buffer, uint16_t bufsize)
{
    ...
    case SCSI_CMD_INQUIRY:
        /*
         * TinyUSB builds a standards-compliant INQUIRY payload from
         * tud_msc_inquiry_cb fields and does not expose a callback API to
         * safely inject an invalid Additional Length byte directly.
         * For case 055, use a controlled class-level reject as the closest
         * safe equivalent for malformed INQUIRY length handling.
         */
        (void)buffer;
        (void)bufsize;
