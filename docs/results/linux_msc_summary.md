...
Цель блока `050-059` — проверить устойчивость Linux USB/MSC-стека при безопасных аномалиях поведения MSC-устройства на RP2040, разделяя успешность USB enumeration/bind и деградацию на уровне SCSI/блочного/FS-слоя.
...
- Доступа к реальным дискам хоста не выполнялось.
...
Счётчики по классификации (только `050-059`):
- `OK`: 1
- `PARTIAL_ENUM`: 6
- `KERNEL_WARNING`: 4
- `EXPECTED_REJECT`: 0
...
## 6. Ограничения доказательной базы
