...
- `053_msc_huge_capacity`: реализован безопасный case-gated runtime-вариант для `USB_CASE_ID=53`, где в `tud_msc_capacity_cb` возвращается нереалистично большой `block_count` (huge capacity) без увеличения фактического RAM-backed носителя; `read10` дополнен строгой проверкой границ и безопасным отклонением out-of-range запросов.
- `054_msc_read_capacity_short_response`: реализован безопасный case-gated runtime-вариант для `USB_CASE_ID=54` с моделированием укороченного/некорректного `READ CAPACITY` через контролируемый protocol-level reject в `tud_msc_scsi_cb`; ограничение TinyUSB: literal short transfer для `READ CAPACITY(10)` напрямую не инжектируется через публичный callback API, поэтому применён ближайший безопасный эквивалент.
- `055_msc_inquiry_invalid_length`: реализован безопасный case-gated runtime-вариант для `USB_CASE_ID=55` в class-command path (`SCSI_CMD_INQUIRY`) как controlled reject для моделирования некорректной длины INQUIRY; ограничение TinyUSB: публичный `tud_msc_inquiry_cb` не даёт безопасно инжектировать literal malformed Additional Length байт стандартного INQUIRY payload, поэтому применён ближайший контролируемый эквивалент без изменения inert/read-only модели.
- `056_msc_csw_invalid_status`: реализован безопасный case-gated runtime-вариант для `USB_CASE_ID=56` как controlled non-good command status эквивалент в `tud_msc_test_unit_ready_cb` (через `sense` + `return false`), поскольку публичный callback API TinyUSB не предоставляет прямого безопасного контроля над literal байтом `CSW status`.
- `057_msc_stall_on_read10`: реализован безопасный case-gated runtime-вариант для `USB_CASE_ID=57` в `tud_msc_read10_cb` как controlled READ(10) failure (`sense` + `return -1`) для моделирования STALL/failure; ограничение TinyUSB: публичный callback API не предоставляет безопасного прямого примитива для literal bulk endpoint STALL из контекста `read10`, поэтому применён ближайший безопасный эквивалент без изменения inert/read-only модели.
- `058_msc_endpoint_packet_size_zero`: реализован безопасный case-gated descriptor-вариант для `USB_CASE_ID=58`, где в MSC configuration descriptor для bulk IN/OUT endpoint задаётся `wMaxPacketSize=0`; изменение ограничено descriptor/enumeration уровнем, runtime read-only RAM-backed политика (`write10` reject) сохранена.
- `059_msc_bot_residue_mismatch`: реализован безопасный case-gated runtime-вариант для `USB_CASE_ID=59` как детерминированный short-data ответ на `READ(10)` в `tud_msc_read10_cb`; ограничение TinyUSB: публичный callback API не предоставляет безопасного прямого контроля literal поля `CSW Data Residue`, поэтому применён ближайший контролируемый эквивалент residue mismatch без изменения inert/read-only модели.
